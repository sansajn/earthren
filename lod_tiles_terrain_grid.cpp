...
vector<terrain> terrain_grid::load_level_tiles(path const & data_path, int level) const {
	...
	// - we expect `.+_elev_C_R.tif` and `.+_rgb_C_R.tif` tile files in tiles_directory
	// - for each elevation tile
	for (fs::directory_entry const & de : find_files(tile_directory, _elevation_tile_prefix)) {
		path const & elevation_file = de.path();  // full path tile filename

		// - parse grid collumn (C) and row (R) position
		regex const tile_pattern{R"(.+(\d+)_(\d+)\.tif)"};  // (column), (row)
		smatch what;
		string const elevation_tile_name = elevation_file.filename().string();
		if (!regex_match(elevation_tile_name, what, tile_pattern))
			continue;  // skip file
		...
		if (!exists(satellite_path)) {
			spdlog::info("corresponding satellite data for elevation tile ('{}') not found", elevation_file.c_str());
			continue;
		}
		...
		// - load elevation tile
		auto const elevation_tile = create_texture_16b(elevation_file);
		...
		trn.elevation_min = _elevation_tile_max_value.at(elevation_file.filename());  // can thrrow std::out_of_range
