...
void draw_terrain(four_terrain_shader_program & shader,
...
void draw_terrain_outlines(GLint shader,
	unsigned int element_count,  // number of quad mesh triengle elements to draw
	vec3 color, GLint outline_line_color_loc,
	GLint outline_heights_loc, GLuint height_map,
	GLint outline_height_map_size_loc, size_t elevation_width, size_t elevation_height,
	float height_scale, GLint outline_height_scale_loc,
	mat4 local_to_screen, GLint outline_local_to_screen_loc,
	render_features const & features) {
	...
	if (features.show_outline) {
		...
		glUniform3fv(outline_line_color_loc, 1, value_ptr(color));
		...
		glUniform2f(outline_height_map_size_loc, elevation_width, elevation_height);
		glUniform1f(outline_height_scale_loc, height_scale);
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char * argv[]) {
	...
	// load textures
	constexpr size_t grid_rows = 1,
		grid_cols = 2;
	// assert((grid_rows % 2) == 0 && (grid_cols % 2) == 0);   // TODOO: why wee need this?
	...
	while (true) {
		...
		// TODO: there we want to draw two tiles next each other and we want a function for draaw
		// - want to see mesh (not care for satellite)
			...
			for (int col = 0; col < static_cast<int>(grid_cols); ++col) {
				...
				if (features.show_terrain) {  // render terrain
					draw_terrain(shader,
						...
						satellite_map, ui.height_scale, elevation_scale, features);
				}
				...
				if (features.show_outline) {  // render wireframe
					draw_terrain_outlines(outline_shader_program,
						...
						outline_height_map_size_loc, texture_width, texture_height,
...
void draw_terrain(four_terrain_shader_program & shader,
	...
	shader.height_map_size(vec2{elevation_width, elevation_height});
...
vector<tuple<GLuint, size_t, size_t>> read_tiles(size_t grid_rows, size_t grid_cols) {
	...
	for (size_t row = 0; row < grid_rows; ++row) {
		for (size_t col = 0; col < grid_cols; ++col) {
			string const height_tile_name = fmt::format("learn/out/tile_{}_{}.tif", col, row);  // TODO: change path
			...
			string const satellite_tile_name = fmt::format("learn/fourtiles/tile_{}_{}.tif", col+1, row+1);  // TODO: change path
