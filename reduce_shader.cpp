/* Sample to write reduce shader program with rendering into framebuffer (FBO) */
#include <string>
#include <tuple>
#include <filesystem>
#include <iostream>
...
out vec2 st;
void main() {
	st = position.xy/2.0 + 0.5;  //= ((0,0), (1,1))
	gl_Position = vec4(position, 1.0f);
})";
...
char const * texture_sample_fs_src = R"(
#version 320 es
precision mediump float;
uniform sampler2D s;
in vec2 st;  //= ((0,0), (1,1))
out vec4 frag_color;
void main() {
	frag_color = texture(s, st);  // texture has twice as much resolution as framebuffer
})";

tuple<GLuint, GLuint, GLuint, unsigned> create_mesh();

// Render texture into bind FBO.
void render_fbo_texture(GLuint color_texture) {
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char * argv[]) {
	...
	// then we want to render this texture into framebuffer object in half resolution
	size_t const fbo_w = texture_w/2.0,
		fbo_h = texture_h/2.0;
	...
	{ // render into framebuffer
		...
		glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT32F, fbo_w, fbo_h);
	...
	render_fbo_texture(color_texture);
